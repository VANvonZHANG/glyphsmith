...
def test_to_path_d_quadratic_offcurve():
    # TrueType convention: off points act as Q controls, and an implied midpoint is
    # inserted between consecutive off points
    o = Outline.from_contours([[(0.0, 0.0, 0), (50.0, 25.0, 1), (100.0, 0.0, 0)]])
    d = o.to_path_d()
    assert "Q 50,25" in d

def test_to_path_d_contour_ending_offcurve():
    # T2 legacy debt: a contour ending on an off-curve point — no more IndexError,
    # it wraps closed to the first on point
...
def test_to_path_d_contour_starting_offcurve():
    # T2 legacy debt: the first point is off-curve — no longer mistaken for an on
    # anchor. Both ends off: it starts from the implied midpoint (and the trailing
    # off point closes onto that midpoint)
    ...
    # first off, last on: rotate, with the last on point as the anchor
